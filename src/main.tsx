...
// Mark that React script loaded and executed successfully
(window as unknown as { reactAppMounted: boolean }).reactAppMounted = true;
...
function MainApp() {
  ...
  useEffect(() => {
    ...
    const handleError = (event: ErrorEvent) => {
      console.error('Captured window error event:', event);
      setError(event.error || new Error(event.message));
    };
    const handleRejection = (event: PromiseRejectionEvent) => {
      console.error('Captured unhandled rejection event:', event);
